...
async function initCommand(projectName, options) {
    console.log(`\n${chalk_1.default.cyan('Creating a new Grishex project:')} ${chalk_1.default.green(projectName)}\n`);
    ...
    try {
        ...
        await fs.writeFile(path.join(projectName, 'grishex.config.json'), JSON.stringify(configContent, null, 2));
        ...
        await fs.writeFile(path.join(projectName, 'package.json'), JSON.stringify(packageJsonContent, null, 2));
        // Create README.md
...
\`\`\`
`;
        await fs.writeFile(path.join(projectName, 'README.md'), readmeContent);
        spinner.succeed('Project structure created successfully');
        // Print success message and next steps
        console.log(`\n${chalk_1.default.green('Success!')} Created ${projectName} at ${path.resolve(projectName)}`);
 ...
 */
async function createSampleContract(projectName, template) {
    let contractContent;
    switch (template) {
        case 'token':
  ...
  function getMessage() public view returns (string memory) {
    ...
    await fs.writeFile(path.join(projectName, 'contracts', contractFileName), contractContent);
}
